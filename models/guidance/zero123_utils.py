...
class Zero123(nn.Module):
    ...
    def get_img_embeds(self, x):
        ...
        # C is clip embedding, V is VAE encoding
        x = x * 2 - 1
        ...
        # print(c[0].shape) # Bs, 1, 768
        # print(v[0].shape) # Bs, 4, 32, 32
        return c, v

    def angle_between(self, sph_v1, sph_v2):
        '''
        sph_v1: [r, theta, phi] (Bs, 3)
        sph_v2: [r, theta, phi] (Bs, 3)
        '''
        def sph2cart(sv):
    ...
    @torch.no_grad()
    def get_angle_scale(self, embeddings, polar, azimuth, radius, grad_scale=1):
        ref_radii = embeddings['ref_radii'] # around 4
        ref_polars = embeddings['ref_polars'] # 90
        ref_azimuths = embeddings['ref_azimuths'] # 0
        ...
        grad_scale = (torch.exp((angles.min(dim=1)[0] / (180/len(ref_azimuths))))-1) * grad_scale 

        return grad_scale

    def train_step(self, embeddings, pred_rgb, polar, azimuth, radius, guidance_scale=3, as_latent=False, grad_scale=1, save_guidance_path:Path=None, t=None, last_grad_scale=None, noise=None):
        ...
        with torch.no_grad():
            ...
            for (zero123_w, c_crossattn, c_concat, ref_polar, ref_azimuth, ref_radius) in zip(zero123_ws.T,
                ...
                noise_pred = self.model.apply_model(x_in, t_in, cond) # [2, 4, 32, 32]
        ...
        if save_guidance_path:
            with torch.no_grad():
                ...
                results_grad = torch.abs(self.decode_latents(noise_pred - noise))
                        
                viz_images = torch.cat([pred_rgb_256, result_noisier_image, result_hopefully_less_noisy_image, results_grad],dim=-1)
                save_guidance_path = save_guidance_path.replace(".png", "_" + str(t.item()) + ".png")
                save_image(viz_images, save_guidance_path)

        targets = (latents - grad).detach()
        loss = 0.5 * F.mse_loss(latents.float(), targets, reduction='sum') / latents.shape[0]
        
        return loss, t, grad_scale, noise
    ...
    def update_t_range(self, t_range):
        self.t_range = t_range
        self.min_step = int(self.num_train_timesteps * t_range[0])
        self.max_step = int(self.num_train_timesteps * t_range[1])
